...
def launch_setup(context, *args, **kwargs):
    ...
    ros2_control_node = Node(
        ...
        remappings=[
            # 네임스페이스 안의 컨트롤러가 전역 혹은 네임스페이스 내의 description을 잘 바라보도록 설정
    ...
    grouped_nodes = GroupAction(
        actions=[
            PushRosNamespace(namespace), # 그룹 내 모든 요소에 네임스페이스 적용
            *nodes_to_start
        ]
    )
...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "namespace",
            default_value="",  # 기본값은 빈 칸(네임스페이스 없음)
            description="GPU namespace for all nodes and topics",
        )
    )
