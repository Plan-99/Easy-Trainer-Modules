...
echo "[robot_unitree] Installing Unitree..."
...
if [ -n "$apt_deps" ]; then
    echo "[robot_unitree] Installing apt packages: $apt_deps"
...
if [ -n "$pip_deps" ]; then
    echo "[robot_unitree] Installing pip packages: $pip_deps"
    python3 -m pip install --quiet $pip_deps || true
fi

echo "[robot_unitree] Installation complete."
