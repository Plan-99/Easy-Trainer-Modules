...
echo "[robot_techman] Installing Techman TM..."
...
if [ -n "$apt_deps" ]; then
    echo "[robot_techman] Installing apt packages: $apt_deps"
...
if [ -n "$pip_deps" ]; then
    echo "[robot_techman] Installing pip packages: $pip_deps"
    python3 -m pip install --quiet $pip_deps || true
fi

echo "[robot_techman] Installation complete."
