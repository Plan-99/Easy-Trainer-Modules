...
echo "[robot_dynamixel] Installing Dynamixel..."
...
if [ -n "$apt_deps" ]; then
    echo "[robot_dynamixel] Installing apt packages: $apt_deps"
...
if [ -n "$pip_deps" ]; then
    echo "[robot_dynamixel] Installing pip packages: $pip_deps"
    python3 -m pip install --quiet $pip_deps || true
fi

echo "[robot_dynamixel] Installation complete."
