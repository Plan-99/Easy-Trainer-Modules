...
echo "[gripper_onrobot] Installing OnRobot Gripper..."
...
if [ -n "$apt_deps" ]; then
    echo "[gripper_onrobot] Installing apt packages: $apt_deps"
...
if [ -n "$pip_deps" ]; then
    echo "[gripper_onrobot] Installing pip packages: $pip_deps"
    python3 -m pip install --quiet $pip_deps || true
fi

echo "[gripper_onrobot] Installation complete."
