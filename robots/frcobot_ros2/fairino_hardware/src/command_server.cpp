...
robot_command_thread::robot_command_thread(const std::string node_name):rclcpp::Node(node_name,
    ...
    /********************************尝试使用SDK库连接机械臂******************************************/
    _controller_ip = this->get_parameter("robot_ip").as_string();//控制器默认ip地址
...
std::string robot_command_thread::ServoMoveStart(std::string para){
    para.clear();
    return std::to_string(_ptr_robot->ServoMoveStart());
}
...
std::string robot_command_thread::ServoMoveEnd(std::string para){
    para.clear();
    return std::to_string(_ptr_robot->ServoMoveEnd());
}

/**
 * @brief 关节空间伺服模式运动
 * @param [in] para - j1,j2,j3,j4,j5,j6,acc,vel,cmdT,filterT,gain
 */
std::string robot_command_thread::ServoJ(std::string para){
    ...
    JointPos joint_pos;
    joint_pos.jPos[0] = std::stod(list.front());list.pop_front();
    joint_pos.jPos[1] = std::stod(list.front());list.pop_front();
    joint_pos.jPos[2] = std::stod(list.front());list.pop_front();
    joint_pos.jPos[3] = std::stod(list.front());list.pop_front();
    joint_pos.jPos[4] = std::stod(list.front());list.pop_front();
    joint_pos.jPos[5] = std::stod(list.front());list.pop_front();

    float acc = list.empty() ? 0.0 : std::stof(list.front()); if(!list.empty()) list.pop_front();
    float vel = list.empty() ? 0.0 : std::stof(list.front()); if(!list.empty()) list.pop_front();
    float cmdT = list.empty() ? 0.008 : std::stof(list.front()); if(!list.empty()) list.pop_front();
    float filterT = list.empty() ? 0.0 : std::stof(list.front()); if(!list.empty()) list.pop_front();
    float gain = list.empty() ? 0.0 : std::stof(list.front());

    ExaxisPos extpos{0,0,0,0};
    return std::to_string(_ptr_robot->ServoJ(&joint_pos, &extpos, acc, vel, cmdT, filterT, gain));
}
