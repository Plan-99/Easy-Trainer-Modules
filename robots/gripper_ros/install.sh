...
echo "[gripper_generic] Installing Generic Gripper..."
...
if [ -n "$apt_deps" ]; then
    echo "[gripper_generic] Installing apt packages: $apt_deps"
...
if [ -n "$pip_deps" ]; then
    echo "[gripper_generic] Installing pip packages: $pip_deps"
    python3 -m pip install --quiet $pip_deps || true
fi

echo "[gripper_generic] Installation complete."
