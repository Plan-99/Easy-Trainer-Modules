...
echo "[robot_piper] Installing Piper..."
...
if [ -n "$apt_deps" ]; then
    echo "[robot_piper] Installing apt packages: $apt_deps"
...
if [ -n "$pip_deps" ]; then
    echo "[robot_piper] Installing pip packages: $pip_deps"
    python3 -m pip install --quiet $pip_deps || true
fi

echo "[robot_piper] Installation complete."
