...
echo "[test_arm] Installing Test Arm..."
...
if [ -n "$apt_deps" ]; then
    echo "[test_arm] Installing apt packages: $apt_deps"
...
if [ -n "$pip_deps" ]; then
    echo "[test_arm] Installing pip packages: $pip_deps"
    python3 -m pip install --quiet $pip_deps || true
fi

echo "[test_arm] Installation complete."
