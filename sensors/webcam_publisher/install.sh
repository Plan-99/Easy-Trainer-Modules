...
echo "[sensor_webcam] Installing Webcam..."
...
if [ -n "$apt_deps" ]; then
    echo "[sensor_webcam] Installing apt packages: $apt_deps"
...
if [ -n "$pip_deps" ]; then
    echo "[sensor_webcam] Installing pip packages: $pip_deps"
    python3 -m pip install --quiet $pip_deps || true
fi

echo "[sensor_webcam] Installation complete."
